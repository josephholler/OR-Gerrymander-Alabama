...
d_ch_est <- blockgroups_calc |> 
  ...
  mutate(
    fArea = st_area(geom),
    awTot = Total * as.numeric(fArea / bgarea),
    awBlack = Black * as.numeric(fArea / bgarea),
  ) |> 
  st_drop_geometry() |> 
...
districts21_join_ch <- districts21_join_bg |> 
  ...
  mutate(chPct = round(chBlack / chTotal * 100, 1),
         chDiff = pctBlackbg - chPct,
         chAbsDiff = abs(chDiff)) |> 
  left_join(d_mbc_est, by = "DISTRICT") |> 
  mutate(mbcPct = round(mbcBlack / mbcTotal * 100, 1),
         mbcDiff = pctBlackbg - mbcPct,
...
districts21_results_cor |> cor() |> corrplot(type="upper", diag=FALSE) 
...
districts21_results_plot <- districts21_results |> 
  st_drop_geometry() |> 
  select(DISTRICT, pctBlack, chAbsDiff, compact_shp, compact_hull, compact_circ) |> 
