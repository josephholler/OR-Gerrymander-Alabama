...
precincts_file <- here("data", "raw", "public", "districts.gpkg")

districts <- st_read(districts_file, layer="districts21")
precincts <- st_read(precincts_file, layer="precincts20")
...
precincts <- precincts |> st_make_valid() |> 
  mutate(sarea = st_area(geom))
```
...
```{r eval=FALSE}
d_ch |> st_write(here("data", "derived", "public", "convex_hull.gpkg"),
                 append = FALSE)
d_mbc |> st_write(here("data", "derived", "public", "minimum_bounding_circle.gpkg"),
                  append = FALSE)
```
...
```{r eval=FALSE}
districts_results |> st_write(here("data", "derived", "public", "district_results_AL.gpkg"),
                              append = FALSE)
```
...
districts_results_cor |> cor() |> corrplot(type="upper", diag=FALSE) 
...
districts_results_plot <- districts_results |> 
  st_drop_geometry() |> 
  select(DISTRICT, pctMinority, chAbsDiff, compact_shp, compact_hull, compact_circ) |> 
...
districts_results_plot |> ggplot() +
  ...
  geom_label(aes(label = DISTRICT, fill = pctMinority)) +
